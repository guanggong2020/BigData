...
class LagouDownloaderMiddleware(object):
    ...
    def __init__(self, agents):
        ...
        for each in json.loads(requests.get(
                url=self.zhima_API,
                headers={"User-Agent": 'Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0)'}).text)[
            'data']:
                ...
                while True:
                    ...
                    try:
                        ...
                        if request.meta['isSchooljobs'] == '0':
                            post_url = f'https://www.lagou.com/jobs/positionAjax.json?jd={self.stage[num]}&px=default&needAddtionalResult=false'
                        else:
                            post_url = f'https://www.lagou.com/jobs/positionAjax.json?jd={self.stage[num]}&px=default&needAddtionalResult=false&isSchoolJob=1'
